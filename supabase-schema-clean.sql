-- Clean setup script for existing Supabase database
-- This script handles existing tables and data safely
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE c.relname = 'articles' AND n.nspname = 'public' AND c.relrowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c 
        JOIN pg_namespace n ON n.oid = c.relnamespace 
        WHERE c.relname = 'admin_users' AND n.nspname = 'public' AND c.relrowsecurity = true
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c 
        JOIN pg_namespace n ON n.oid = c.relnamespace 
        WHERE c.relname = 'admin_sessions' AND n.nspname = 'public' AND c.relrowsecurity = true
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'articles' AND policyname = 'Articles are insertable by authenticated users') THEN
        CREATE POLICY "Articles are insertable by authenticated users" ON articles
          FOR INSERT WITH CHECK (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'articles' AND policyname = 'Articles are updatable by authenticated users') THEN
        CREATE POLICY "Articles are updatable by authenticated users" ON articles
          FOR UPDATE USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'articles' AND policyname = 'Articles are deletable by authenticated users') THEN
        CREATE POLICY "Articles are deletable by authenticated users" ON articles
          FOR DELETE USING (auth.role() = 'authenticated');
    END IF;
    
    -- Admin users policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_users' AND policyname = 'Admin users are viewable by service role') THEN
        CREATE POLICY "Admin users are viewable by service role" ON admin_users
          FOR ALL USING (auth.role() = 'service_role');
    END IF;
    
    -- Admin sessions policies
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'admin_sessions' AND policyname = 'Admin sessions are viewable by service role') THEN
